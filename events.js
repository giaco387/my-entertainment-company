...
const EVENTS = [
  ...
  },
  {
    ...
    cost:0, minArtists:1, statHint:'舆论值', duration:1,
    req:'需出道艺人 ≥1',
    check()    { return G.artists.some(a => a.status === '已出道' || a.status === '工作中'); },
    artistCheck(a) { return a.status === '已出道' || a.status === '工作中'; },
    compute(sel) {
...
const MONTHLY_EVENTS = {
  industry: [
    ...
    { name:'跨界联名热潮',        icon:'🤝', type:'good', desc:'品牌联名风潮带动代言市场，公司声望提升',
    ...
    { name:'行业监管突然收紧',    icon:'🚨', type:'bad',  desc:'政策收紧影响演出市场，收入和声望双降',
